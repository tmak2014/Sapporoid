...
jint
//Java_robo_servotester_ServoControl_I2cGet( JNIEnv* env, jobject thiz, jintArray arr )
Java_robo_sapporoid_ServoControl_I2cGet( JNIEnv* env, jobject thiz, jstring port, jstring addr, jstring num )
{
	char *end;
...
//		exit(0);
	...
	if( retFlag == 0 ){
		...
		if( check_funcs(file, size, daddress, pec) ){
