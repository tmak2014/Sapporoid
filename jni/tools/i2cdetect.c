...
jint
Java_robo_sapporoid_ServoControl_I2cDetect( JNIEnv* env, jobject thiz, jint x )
{
	char *end;
	int i2cbus, file, res;
	char filename[20];
	...
	int argc = 4;
//	char argv[5][2] = {{'0','0'}, {'-','y'}, {'-','r'},{'2', 0x00}, {0x00, 0x00}};
	char arg0[] = {"0"};
	char arg1[] = {"-y"};
	char arg2[] = {"-r"};
	char arg3[] = {"2"};
	...
	i2cLog("SVT1", "SVT2","I2cDetect Start");
	i2cLog("SVT1", "SVT2","#1");
	/* handle (optional) flags first */
	while (1+flags < argc && argv[1+flags][0] == '-') {
		i2cLog("SVT1", "SVT2", &argv[1+flags][0]);
		i2cLog("SVT1", "SVT2", &argv[1+flags][1]);
...
//			exit(1);
	...
	i2cLog("SVT1", "SVT2", ci2cbus);
	file = open_i2c_dev(i2cbus, filename, 0);
//	sprintf(filename, "/dev/i2c-%d", i2cbus);
//	file = open(filename, O_RDWR);
	if (file < 0) {
		sprintf(ci2cbus, "#21 %d", file);
		i2cLog("SVT1", "SVT2", ci2cbus);
//			exit(1);
		return x;
	}
...
//			exit(1);
	...
	if (!yes) {
		...
		if (!fgets(s, 2, stdin)
		 || (s[0] != '\n' && s[0] != 'y' && s[0] != 'Y')) {
			__android_log_write("SVT1", "SVT2","#27");
...
//	exit(res?1:0);
	...
	i2cLog("SVT1", "SVT2","I2cDetect End");
	return x + 1;
}
